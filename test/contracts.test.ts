/**
 * Mantlic Smart Contract Tests
 * Tests for MantlicAgentRegistry and MantlicSwap
 * 
 ...
 await registry.waitForDeployment();
  ...
  describe("Reputation Registry", function () {
    ...
    it("should get normalized reputation", async function () {
      ...
      expect(normalized).to.equal(5500); // 5000 + 5000 offset
    });
    ...
    it("should record transaction outcomes", async function () {
      ...
      expect(stats.successfulTx).to.equal(1);
    });
  });

  describe("Validation Registry", function () {
    it("should create attestation", async function () {
      const { registry, owner, agent1, agent2 } = await loadFixture(deployRegistry);
      
      // Register two agents
      await registry.registerAgent("Agent 1", "ipfs://Qm1", "0x");
      await registry.connect(agent1).registerAgent("Agent 2", "ipfs://Qm2", "0x");
      
      // Agent 1 attests to Agent 2
      await registry.connect(agent1).attestAgent(1, 2, 85, "Good trading performance");
      
      const attestations = await registry.getAttestations(2);
    ...
    it("should calculate average attestation score", async function () {
      ...
      await registry.connect(agent1).attestAgent(1, 2, 80, "Note 1");
      
      const avgScore = await registry.getAverageAttestationScore(2);
  ...
  describe("Benchmarking", function () {
    ...
    it("should get latest benchmark by type", async function () {
      ...
      await registry.recordBenchmark(1, "sentiment_analysis", 7000, 100, 0.02e18, "ipfs://sa1");
      await registry.recordBenchmark(1, "sentiment_analysis", 8000, 80, 0.01e18, "ipfs://sa2");
...
describe("MantlicSwap", function () {
  ...
  describe("Swap Execution", function () {
    ...
    it("should revert on same token swap", async function () {
      const { swap, user } = await loadFixture(deploySwap);
      const token = "0x0000000000000000000000000000000000000001";
      
      await expect(
        swap.executeSwap(token, token, ethers.parseEther("1"), 0, "0x")
  ...
  describe("Admin Functions", function () {
    it("should update fee", async function () {
      const { swap, deployer } = await loadFixture(deploySwap);
